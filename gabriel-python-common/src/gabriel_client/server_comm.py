...
class WebsocketClient:
    ...
    async def _consumer_handler(self):
        try:
            while self._running:
                ...
                to_client.timestamps.received = time_receive

                if to_client.HasField('result_wrapper'):
                    #to_client.result_wrapper.s2c_timestamps = to_client.timestamps
                    result_wrapper = to_client.result_wrapper
                    #result_wrapper.s2c_timestamps = to_client.timestamps
                    result_wrapper.s2c_timestamps.CopyFrom(to_client.timestamps)
    ...
    async def _send_helper(self, from_client):
        ...
        from_client.timestamps.sent = time.time()
